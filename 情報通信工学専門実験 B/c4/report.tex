...
周波数 $1\,\mathrm{MHz}$ を中心とし，$0.9\,\mathrm{MHz}$ と $1.1\,\mathrm{MHz}$ の位置にピーク成分を有するスペクトル波形が存在することがわかる。

実験テキストにおける振幅変調信号の定義は \cite{2}
\[
x_{\mathrm{AM}}(t) = m(t)\cos \omega_c t
\]
である。本実験では，変調信号 $m(t)$ として正弦波
\[
m(t) = A_m \sin(\omega_m t)
\]
を用いているため，この定義に従うと，本実験で解析すべき信号は
\[
x_{\mathrm{AM}}(t) = A_m \sin(\omega_m t)\cos \omega_c t
\]
となる。

さらに，搬送波には振幅係数 $A_c$ が掛かっているので，実際に観測される信号は
\[
A_m \sin(\omega_m t)\,A_c \cos(\omega_c t)
\]
と表される。以下ではこの信号のフーリエ変換を求め，側波帯の周波数成分を確認する。
...
\begin{align*}
A_m \sin(\omega_m t)\,A_c \cos(\omega_c t)
&= \frac{A_m A_c}{2} \left\{\sin\bigl((\omega_c+\omega_m)t\bigr)+\sin\bigl((\omega_c-\omega_m)t\bigr)\right\} \\
&= \frac{A_m A_c}{4i} \Bigl\{e^{i(\omega_c+\omega_m)t} - e^{-i(\omega_c+\omega_m)t} \\
&\hspace{6em} + e^{i(\omega_c-\omega_m)t} - e^{-i(\omega_c-\omega_m)t}\Bigr\}
\end{align*}

$\delta(t-t_0)\Longleftrightarrow e^{-i\omega_0 t}$ およびフーリエ変換の対称性より，
$e^{i\omega_0 t}\Longleftrightarrow 2\pi \delta(\omega-\omega_0)$ が成り立つので，
\begin{align*}
\mathcal{F}\bigl[A_m \sin(\omega_m t)\,A_c \cos(\omega_c t)\bigr]
&= \frac{\pi A_m A_c}{2i} \Bigl\{\delta\bigl(\omega-(\omega_c+\omega_m)\bigr)
+ \delta\bigl(\omega-(\omega_c-\omega_m)\bigr) \\
&\hspace{7em} - \delta\bigl(\omega+(\omega_c+\omega_m)\bigr)
- \delta\bigl(\omega+(\omega_c-\omega_m)\bigr)\Bigr\} \\
&= \frac{\pi A_m A_c}{2i} \Bigl\{\delta\bigl(2\pi(f-f_c-f_m)\bigr)
+ \delta\bigl(2\pi(f-f_c+f_m)\bigr) \\
&\hspace{7em} - \delta\bigl(2\pi(f+f_c+f_m)\bigr)
- \delta\bigl(2\pi(f+f_c-f_m)\bigr)\Bigr\} \quad \cdots (1)
\end{align*}

ここで $\omega = 2\pi f,\ \omega_c = 2\pi f_c,\ \omega_m = 2\pi f_m$ とした。
本実験において $f_c = 1\,\mathrm{MHz}$，$f_m = 0.1\,\mathrm{MHz}$ である。$f_c > f_m$ であることに注意すると，式 (1) より，$f \ge 0$ において周波数 $f = f_c \pm f_m$ の位置にピークが現れることが分かる。したがって，$1\,\mathrm{MHz}$ を中心とし，$0.9\,\mathrm{MHz}$ と $1.1\,\mathrm{MHz}$ の 2 箇所でピーク成分を有すると考えられる。
...
矩形波振幅変調のスペクトル波形からは，$1\,\mathrm{MHz}$ を中心とし，$100\,\mathrm{kHz}$ ごとにパワーがゼロとなる点が現れ，中心から離れるほど減衰していく $\operatorname{sinc}$ 関数のような波形が確認できた。

このような特徴が生じた理由を，実験テキストの定義 \cite{2}
\[
x_{\mathrm{AM}}(t) = m(t)\cos \omega_c t
\]
に
\[
m(t) = A_m \operatorname{rect}\!\left(\frac{t}{T_m}\right)
\]
を代入した信号のフーリエ変換を調べることで確認する。

ここで，変調信号を
\[
f(t) = A_m \operatorname{rect}\!\left(\frac{t}{T_m}\right),
\]
搬送波を
\[
g(t) = A_c \cos \omega_c t
\]
とおくと，扱う信号は $f(t)g(t)$ であり，そのフーリエ変換は畳み込みの定理により
\[
\mathcal{F}[f(t)g(t)] = F(\omega) * G(\omega)
\]
と書ける。$f(t)$ のフーリエ変換 $F(\omega)$ は $T_m$ 幅の矩形波に対応する $\operatorname{sinc}$ 関数となり，$g(t)$ のフーリエ変換 $G(\omega)$ は $\omega = \pm \omega_c$ にデルタ関数を持つスペクトルとなる。両者の畳み込みを計算すると，次のような結果が得られる。
\[
\mathcal{F}\!\left[A_m \operatorname{rect}\!\left(\frac{t}{T_m}\right) A_c \cos \omega_c t\right]
= \frac{A_m A_c T_m}{2}
\left[
\operatorname{sinc}\!\left(T_m \pi \left(f - \frac{1}{T_c}\right)\right)
+
\operatorname{sinc}\!\left(T_m \pi \left(f + \frac{1}{T_c}\right)\right)
\right]
\quad \cdots (4)
...
式 (4) より，このスペクトルは $f \ge 0$ において，中心が $1\,\mathrm{MHz}$，周期が $100\,\mathrm{kHz}$ の $\operatorname{sinc}$ 波形となることがわかる。それぞれの $\operatorname{sinc}$ 関数は $100\,\mathrm{kHz}$ 周期で値が 0 になり，これは実験結果と一致する。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.4\textwidth]{./C4_位相変調 2/せいげんは/３ｖ/Screen_0002.png}
...
私の役割は、班全体の進行をマネジメントすることであった。実験開始時には各メンバーの自己紹介を行い、その後、作業手順の確認と役割分担を調整しながら、実験が円滑に進むよう統括した。

特に力を入れた点は、役割の明確化とデータ整理の仕組みづくりである。これにより、実験中の作業効率が向上しただけでなく、後続のレポート作成がスムーズに行える体制を整えることができた。

実務面では、Excel を用いて波形データの作成を担当し、班内で共有すべき基盤データを整備した。



\begin{thebibliography}{99}
\bibitem{1} オシロスコープの読み方を参考にした。具体的には、オシロスコープの縦軸スケールを示すのはどの部分かを調査した。\\
\texttt{https://download.tek.com/manual/TBS1000B-and-TBS1000B-EDU-Oscilloscope-User-Manual-JA-JA.pdf}

\bibitem{2} C4 信号変調の基礎実験 実験テキスト
\end{thebibliography}
